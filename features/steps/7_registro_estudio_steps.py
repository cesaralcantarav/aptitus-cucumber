...
def postulante_selecciona_opc_estudio(context):
    ...
    estudio_opc = context.web.find_by_xpath("//*[@id='body']/div[1]/div/div/div[1]/a[5]")
    estudio_opc.click()

@when(u"postulante hace click en boton Agregar Estudio")
...
def postulante_selecciona_grado(context, grado):
    grade_field = context.web.find_by_id("gradeIdstudies")
    grade_field.send_keys(str(grado))
...
def postulante_ingresa_institucion(context, institucion):
    institution_field = context.web.find_by_id("institutionstudies")
    institution_field.send_keys(str(institucion))

@when(u"postulante selecciona primera opcion segun su institucion")
def posutlante_selecciona_institucion(context):
...
def postulante_selecciona_ms_ini(context, mes_inicio):
    start_month_est_field = context.web.find_by_id("startMonthstudies")
    start_month_est_field.send_keys(str(mes_inicio))

@when(u"postulante selecciona un anio inicio de estudio {anio_inicio}")
def postulante_selecciona_anio_ini(context, anio_inicio):
    start_year_est_field = context.web.find_by_id("startYearstudies")
    start_year_est_field.send_keys(str(anio_inicio))

@when(u"postulante selecciona un mes fin de estudio {mes_fin}")
def postulante_selecciona_ms_fin(context, mes_fin):
    end_month_est_field = context.web.find_by_id("endMonthstudies")
    end_month_est_field.send_keys(str(mes_fin))

@when(u"postulante selecciona un anio fin de estudio {anio_fin}")
def postulante_selecciona_anio_fin(context, anio_fin):
    end_year_est_field = context.web.find_by_id("endYearstudies")
    end_year_est_field.send_keys(str(anio_fin))
...
@then(u"postulante visualiza tooltip de confirmacion de Estudio Guardado {mensaje}")
